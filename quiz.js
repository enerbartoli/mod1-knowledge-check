...
function renderPendingUsers() {
  ...
  if (toggleEl) {
    ...
    [
      ...
      btn.className = 'pending-type-btn' + (active ? ' active' : '');
      btn.dataset.type = t.key;
      btn.style.cssText = `padding:7px 18px;font-size:12px;font-weight:700;border-radius:20px;cursor:pointer;` +
      ...
      btn.innerHTML = `${t.label} <span style="background:${active ? 'rgba(0,0,0,.15)' : 'var(--navy-light)'};border-radius:10px;padding:1px 7px;margin-left:4px;">${t.count}</span>`;
      btn.addEventListener('click', () => { pendingViewType = t.key; renderPendingUsers(); });
      toggleEl.appendChild(btn);
    });
    // clear and re-render toggle each time
    toggleEl.innerHTML = '';
    [
      { key: 'never',  label: 'Never taken the test', count: nTotal  },
      { key: 'failed', label: 'Taken — not yet passed', count: fTotal }
